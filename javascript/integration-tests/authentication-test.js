...
(async () => {
    ...
    const server = new Expector("./tsc.out/implementation/main.js", [], { env: { GINK_PORT: "8081", GINK_AUTH_KEY: "token abc", ...process.env } });
    ...
    await client.send("await database.connectTo('ws://127.0.0.1:8081', {authToken:'token abc'}).catch((err)=>console.error(err, 'unable to connect'));\n");
    await server.expect("Connection accepted.", 1000);
