...
(async () => {
    ...
    server = false && new Expector("./tsc.out/implementation/main.js", ["-l", port], {
        env: { ...process.env },
    });
    ...
    await Directory.get(instance1).set("foo", "bar", {comment: "testing peer callback"});
    console.log("wrote to first instance");
    await lbstore1.close();
